...
class TopicTracker:
    ...
    def __init__(self, topic_keywords: Dict[str, List[str]] = None):
        self.topic_keywords = topic_keywords or self._default_marketing_keywords()
        self.discussed_topics: List[str] = []

    def _default_marketing_keywords(self) -> Dict[str, List[str]]:
        """Default marketing topic keywords."""
        return {
            'roi': ['roi', 'return on investment', 'roas', 'profitability'],
            'campaigns': ['campaign', 'ads', 'advertising', 'creative'],
            'channels': ['channel', 'paid', 'organic', 'social', 'email'],
            'content': ['content', 'blog', 'video', 'article', 'post'],
            'conversion': ['conversion', 'funnel', 'leads', 'pipeline'],
            'budget': ['budget', 'spend', 'investment', 'cost'],
            'benchmarks': ['benchmark', 'compare', 'industry', 'competitors'],
            'attribution': ['attribution', 'tracking', 'measurement'],
            'engagement': ['engagement', 'interaction', 'reach', 'impressions'],
            'branding': ['brand', 'awareness', 'positioning', 'messaging'],
        }
...
def create_topic_tracker(product_type: str = 'marketing') -> TopicTracker:
    """Create topic tracker with product-specific keywords."""
    keywords = None

    if product_type == 'security':
        keywords = {
            'vulnerabilities': ['vulnerability', 'cve', 'patch', 'exploit'],
            'incidents': ['incident', 'breach', 'attack', 'compromise'],
            'compliance': ['compliance', 'audit', 'framework', 'regulation'],
            'risk': ['risk', 'threat', 'exposure', 'impact'],
            'access': ['access', 'identity', 'authentication', 'mfa'],
            'threats': ['threat', 'ransomware', 'malware', 'phishing'],
            'posture': ['posture', 'security score', 'maturity'],
            'network': ['network', 'firewall', 'segmentation'],
            'endpoint': ['endpoint', 'edr', 'antivirus', 'device'],
        }
    elif product_type == 'executive':
        keywords = {
            'goals': ['goal', 'objective', 'okr', 'target', 'milestone'],
            'financial': ['revenue', 'profit', 'margin', 'cash', 'runway'],
            'customers': ['customer', 'nps', 'churn', 'retention'],
            'people': ['employee', 'team', 'engagement', 'turnover'],
            ...
            'operations': ['operation', 'efficiency', 'process'],
        }

    return TopicTracker(topic_keywords=keywords)
